...
public class treeMap {
    ...
    public static void main(String[] args) {
        ...
        map.firstKey();                         // Get the first (lowest) key
        map.lastKey();                          // Get the last (highest) key
        map.lowerKey("Banana");                 // Get the greatest key less than "Banana"
        map.higherKey("Banana");                // Get the least key greater than "Banana"
        map.floorKey("Banana");                 // Get the greatest key less than or equal to "Banana"
        map.ceilingKey("Banana");               // Get the least key greater than or equal to "Banana"

        map.firstEntry();                       // Get the first entry
        map.lastEntry();                        // Get the last entry
        map.lowerEntry("Banana");               // Get the greatest entry less than "Banana"
        map.higherEntry("Banana");              // Get the least entry greater than "Banana"
        map.floorEntry("Banana");               // Get the greatest entry less than or equal to "Banana"
        map.ceilingEntry("Banana");             // Get the least entry greater than or equal to "Banana"

        //Submaps and Ranges
        map.subMap("Apple", "Cherry");          // Get a submap from "Apple" (inclusive) to "Cherry" (exclusive)
        map.headMap("Banana");                  // Get a view of the portion of the map whose keys are less than "Banana"
        map.tailMap("Banana");                  // Get a view of the portion of the map whose keys are greater than or equal to "Banana"
        ...
        Map<String, Integer> synchronizedMap = Collections.synchronizedMap(map); // Synchronize map
        Map<String, Integer> unmodifiableMap = Collections.unmodifiableMap(map); // Make map read-only
